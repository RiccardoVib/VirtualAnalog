...
from ProperEvaluationAllModels import load_model_lstm_enc_dec, load_model_lstm_enc_dec_v2, inferenceLSTM_enc_dec, inferenceLSTM_enc_dec_v2, load_model_attention, load_model_v3
from ProperEvaluationAllModels import plot_time, plot_fft, create_ref, spectrogram, load_ref, load_model_GRU
import os
import pickle
import glob
import audio_format
...
def retrive_info(architecture, model_dir, units, drop, w):
    ...
    if architecture == 'attention':
        ...
        model = load_model_attention(T=T, D=D, encoder_units=enc_units, decoder_units=dec_units, drop=drop, model_save_dir=data_dir)
        #time_s = measure_time(model=model, x_test=x_test, y_test=x_test, enc_dec=True, v2=True, data_dir=data_dir, fs=fs, scaler=scaler, T=T)
        #print(time_s)
        sig_name = ['_sweep_', '_guitar_', '_drumKick_', '_drumHH_', '_bass_']
        sec = [32, 135, 238, 240.9, 308.7]
        ...
        for l in range(len(sig_name)):
            ...
            audio_tar = audio_tar[w:]
            audio_pred = audio_format.pcm2float(audio_pred)
            plot_time(audio_tar, audio_pred, audio_inp, fs, data_dir, 'attention' + sig_name[l])
            plot_fft(audio_tar, audio_pred, audio_inp, fs, data_dir, 'attention' + sig_name[l])
            results = measure_performance(audio_tar, audio_pred, name)
            all_results.append(results)
    ...
    if architecture == 'v3':
        ...
        model = load_model_v3(T=T, D=D, encoder_units=enc_units, decoder_units=dec_units, drop=drop, model_save_dir=data_dir)
        #time_s = measure_time(model=model, x_test=x_test, y_test=x_test, enc_dec=True, v2=True, data_dir=data_dir, fs=fs, scaler=scaler, T=T)
        #print(time_s)
        sig_name = ['_sweep_', '_guitar_', '_drumKick_', '_drumHH_', '_bass_']
        sec = [32, 135, 238, 240.9, 308.7]
    ...
    if architecture == 'GRU':
        ...
        model = load_model_GRU(T=T, D=D, encoder_units=enc_units, decoder_units=dec_units, drop=drop, model_save_dir=data_dir)
        #time_s = measure_time(model=model, x_test=x_test, y_test=x_test, enc_dec=True, v2=True, data_dir=data_dir, fs=fs, scaler=scaler, T=T)
        #print(time_s)
        sig_name = ['_sweep_', '_guitar_', '_drumKick_', '_drumHH_', '_bass_']
        sec = [32, 135, 238, 240.9, 308.7]
...
if __name__ == '__main__':
    ...
    #retrive_info(architecture='lstm_enc_dec_v2', model_dir='ED_comb', units=[64, 64], drop=0., w=16)
    #retrive_info(architecture='attention', model_dir='ED_att', units=[64, 64], drop=0., w=16)
    #retrive_info(architecture='v3', model_dir='ED_v3', units=[64, 64], drop=0., w=16)
    retrive_info(architecture='GRU', model_dir='ED_GRU', units=[64, 64], drop=0., w=16)