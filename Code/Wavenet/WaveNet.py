...
class WaveNet(tf.keras.Model):
    def __init__(self, dilation_rate, kernel_size, filters, residual_channels, gate_channels):
        super().__init__()

        #self.upsample_network = ops_2.UpsampleNetwork(upsample_scales)

        self.first_layer = ops_2.Conv1D(128,
                                  kernel_size=1,
                                  padding='causal')

        self.residual_blocks = []
        for _ in range(2):#number of activation so sigmoid and tanh
            for i in range(10):#number of layer
                self.residual_blocks.append(
                    ops_2.ResidualConv1DGLU(128,
                                      256,
                                      kernel_size=3,
