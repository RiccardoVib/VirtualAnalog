...
class WaveNetModel(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.variablesNet = self._create_variables()


    def call(self, inputs, global_condition_batch=None, name='wavenet'):
        inp, tar = inputs
        with tf.name_scope(name):
            # We mu-law encode and quantize the input audioform.
            #encoded_input = mu_law_encode(inp, self.quantization_channels)
            #encoded_target = mu_law_encode(tar, self.quantization_channels)
            encoded_input = inp #(n_batches, time, freq)
            encoded_target = tar
            ...
            #target_output = tf.slice(tf.reshape(encoded_target, [self.batch_size, -1, self.quantization_channels]), [0, self.receptive_field, 0], [-1, -1, -1])
            target_output = encoded_target
            prediction = tf.keras.activations.sigmoid(raw_output)
    ...
    def _create_causal_layer(self, input_batch):
        ...
        with tf.name_scope('causal_layer'):
            weights_filter = self.variablesNet['causal_layer']['filter']
    ...
    def _create_dilation_layer(self, input_batch, layer_index, dilation,
        ...
        '''
        variables = self.variablesNet['dilated_stack'][layer_index]
    ...
    def _create_network(self, input_batch, global_condition_batch):
        ...
        with tf.name_scope('postprocessing'):
            ...
            w1 = self.variablesNet['postprocessing']['postprocess1']
            w2 = self.variablesNet['postprocessing']['postprocess2']
            if self.use_biases:
                b1 = self.variablesNet['postprocessing']['postprocess1_bias']
                b2 = self.variablesNet['postprocessing']['postprocess2_bias']
    ...
    def _create_generator(self, input_batch, global_condition_batch):
        ...
        with tf.name_scope('postprocessing'):
            variables = self.variablesNet['postprocessing']
            # Perform (+) -> ReLU -> 1x1 conv -> ReLU -> 1x1 conv to
            # postprocess the output.
    ...
    def _embed_gc(self, global_condition):
        ...
        if self.global_condition_cardinality is not None:
            ...
            embedding_table = self.variablesNet['embeddings']['gc_embedding']
    ...
    def loss(self,
        ...
        with tf.name_scope(name):
            ...
            encoded_tar = self._one_hot(encoded_target)
            if self.scalar_input:
                network_input = tf.reshape(
                    tf.cast(input_batch, tf.float32),
                    [self.batch_size, -1, 1])
                    #network_output = tf.reshape(
                    #tf.cast(target_batch, tf.float32),
                    #[self.batch_size, -1, 1])
            else:
                network_input = encoded
                #network_output = encoded_tar
