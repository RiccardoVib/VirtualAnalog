...
def threshold_loss(lab, pred, thres=.08, zero_value=0.):
    # locs_present = tf.where(tf.math.logical_or(lab > (zero_value + thres), lab < (zero_value - thres)))
    locs_present = tf.where(lab >= (zero_value + thres))
    loss_thres = tf.reduce_mean(tf.abs(tf.gather_nd(lab, locs_present) - tf.gather_nd(pred, locs_present))**2)

    # locs_present = tf.where(tf.math.logical_and(lab < (zero_value + thres), lab > (zero_value - thres)))
    locs_present = tf.where(lab < (zero_value + thres))
    loss_mae = tf.reduce_mean(tf.abs(tf.gather_nd(lab, locs_present) - tf.gather_nd(pred, locs_present))**2)

    return loss_thres, loss_mae
...
def train_RAMT(data_dir, epochs, seed=422, data=None, **kwargs):
    ...
    def val_step(inp, tar, testing=False):
        ...
        loss_thres, loss_sthres = threshold_loss(tar_real, predictions, zero_value=0.)
        loss = loss_thres + loss_sthres
        loss_mae = loss_fn(tar_real, predictions)
    ...
    if ckpt_flag:
        ...
        if os.path.exists(save_model_latest):
            ...
            transformer([tf.costant(Z[x[:2]], dtype='float32'),
...
if __name__ == '__main__':
    ...
    train_Longformer(
        ...
        data=data,
        model_save_dir='../../TrainedModels',
        save_folder='Longformer_TESTING',
        ckpt_flag=True,
        plot_progress=True,
        learning_rate=0.001,
        b_size=16,
        num_layers=4,
        d_model=32,
        attention_window=[64, 64, 64, 64],
        dff=32,
        num_heads=2,
        drop=0.1,
        global_attention_prob=.1,
        epochs=100,
        seed=422,
        generate_wav=2)