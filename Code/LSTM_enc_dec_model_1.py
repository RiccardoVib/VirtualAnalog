...
def trainLSTM(data_dir, epochs, seed=422, data=None, **kwargs):
    ...
    if len(encoder_units) > 0:
        ...
        outputs, state_h, state_c = LSTM(last_unit_encoder, return_sequences=False, return_state=True, name='LSTM_EnFin')(outputs)
    else:
        outputs, state_h, state_c = LSTM(first_unit_encoder, return_sequences=False, return_state=True, name='LSTM_En')(encoder_inputs)
    ...
    if len(decoder_units) > 0:
        ...
        outputs = LSTM(last_unit_decoder, return_sequences=True, return_state=False, name='LSTM_DeFin',
                             dropout=drop)(outputs)
    else:
        outputs = LSTM(first_unit_decoder, return_sequences=True, return_state=False, name='LSTM_De',
                             dropout=drop)(
            decoder_inputs,
            initial_state=encoder_states)
    if drop != 0.:
    ...
    final_model_test_loss = model.evaluate([x_test[:, :-1, :], x_test[:, -1, 0].reshape(-1, 1)], y_test[:, -1],
