...
import Transformer
from Code.TrainFunctionality import CustomSchedule, PlotLossesSame
...
def train_RAMT(data_dir, epochs, seed=422, data=None, **kwargs):
    ...
    # Get the data:
    if data is None:
        x, y, x_val, y_val, x_test, y_test, scaler, zero_value = get_data_piano(data_dir,
    ...
    # Get batches
    x_batches, y_batches = x_test, y_test
    for batch_num in range(len(x_batches)):
        x_batch = x_batches[batch_num * b_size:batch_num * b_size + b_size]
        y_batch = y_batches[batch_num * b_size:batch_num * b_size + b_size]
    ...
    predictions, _ = transformer([tf.constant(x_test, dtype='float32'), tf.constant(y_test[:, :-1], dtype='float32')], training=False)
    predictions = predictions.numpy()

    y_s = np.reshape(y_test, (-1))
    y_pred = np.reshape(predictions, (-1))
    r_squared = 0#coefficient_of_determination(y_s[:1600], y_pred[:1600])
...
if __name__ == '__main__':
    ...
    train_RAMT(
        ...
        save_folder='Transformer_piano_TESTING_imp',
        ckpt_flag=True,
        plot_progress=True,
        b_size=128,
        num_layers=2,
